...
struct NPath 
{
	...
	NPath (vector < int > _perm, vector < Path > _paths) // calculates weight by multiplying weight of path
	{
		perm = _perm;
		paths = _paths;
		weight = Polynomial(1);
	...
	{
		perm.resize(max((int)perm.size(), s + 1));
		paths.resize(max((int)paths.size(), s + 1));
